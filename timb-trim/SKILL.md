...
description: >-
  ...
  just the current diff vs main. Writes findings to a markdown report, asks for
...
- **Public API stays public.** Anything exported across a package/module boundary that external code could depend on is off-limits for renames unless the user explicitly opts in for that finding. How to decide what's public, by language:
...
- `Diff vs main` (recommended) — files touched on this branch (`git diff --name-only main...HEAD` + uncommitted from `git status`).
- `Whole codebase` — every source file the repo's ignore rules permit.

Record the file list. From here on, "in scope" means that list.

**Always exclude**, even from the whole-codebase scope:

- `vendor/`, `third_party/`, `node_modules/` (even if checked in)
- `dist/`, `build/`, `out/`, `target/`, `.next/`, `coverage/`
- Generated files by pattern: `*.gen.*`, `*.pb.*`, `*_pb2.py`, `*.min.*`, `*-lock.*`, `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `Cargo.lock`, `go.sum`
...
- Go: `interface{}` → `any`; manual loops `slices`/`maps` covers; `ioutil` → `io`/`os`; superfluous nil checks; `fmt.Errorf` without `%w` when wrapping.
...
**c. Dead code.** Unreferenced exports, unused imports, unreachable branches, commented-out blocks, never-called methods. Cross-check with `grep`/LSP before flagging — fewer false positives than guessing.
...
Scope: <diff vs main | whole codebase>
